...
import org.example.viptravel.view.tdm.PaymentTM;
...
public class PaymentFormController {
    ...
    public  void getCurrentID(){
        try {
            String curentID = paymentBO.getCurrentPaymentID();
    ...
    public void btnSavePay(ActionEvent event) {
        ...
        PaymentDTO paymentModel = new PaymentDTO(paymentid, status, type, fullPayment, reservationID, paymentDate,PaymentMethod,AdvancedPay,balancedPay);

        try {
            boolean isSaved = paymentBO.addPayment(paymentModel);
            if (isSaved) {
    ...
    private void loadAllPayments() {
        ...
        try {
            List<PaymentDTO> paymentModels = paymentBO.getAllPayment();

            for (PaymentDTO paymentModel : paymentModels) {
   ...
   public void NicOnAction(ActionEvent event) {
       ...
       if (!NIC.isEmpty()) {
           try {
               List<Payment> paymentList = paymentBO.getPaymentsByNIC(NIC);
    ...
    private void loadPayments(List<Payment> paymentList) {
       tblPayment.getItems().clear();
        for (Payment paymentModel : paymentList) {
            PaymentTM paymentTM = new PaymentTM(
                    paymentModel.getPaymentID(),
                    paymentModel.getStatus(),
                    paymentModel.getType(),
    ...
    public void setCompletePayment()  {
        ...
        try {
          //  PaymentRepo paymentRepo = new PaymentRepo();
            PaymentDTO paymentModle = new PaymentDTO();

            int completePayment = paymentBO.getCompletePayment(paymentModle);
    ...
    public void setPendingPayment()  {
        try {
          //  PaymentRepo paymentRepo = new PaymentRepo();
            ...
            int PendingPayment = paymentBO.getPendingPayment(paymentModle);
